...
description: 사용자가 `$` 없이 8개 라우트 중 하나인 `사무실-코덱스-그록!` 또는 `집-클로드-딥시크!`처럼 입력한 라우트 자체를 호출명으로 받아 장소·오퍼레이터·라이터를 확정하는 SSWCenter 3.0 라우팅 스킬이다. 지정된 조합의 경로·등급·독립 작업방·수동 Git 흐름을 적용한다.
---
...
이 스킬은 `$`로 스킬명을 붙이지 않아도 라우트 문자열 뒤에 `!`를 붙여 호출한다. `!`는 호출 마커이며 정규화된 장소·오퍼레이터·라이터 값에는 포함하지 않는다. 라우트 뒤에 작업 내용을 이어 쓰거나, 라우트만 먼저 입력해 현재 대화의 활성 조합을 정할 수 있다.
...
라우트와 작업을 한 번에 입력하는 경우:

```text
사무실-코덱스-그록! 이 작업을 분석하고 구현해
집-클로드-딥시크! 이 버그를 수정해
```

기존의 명시적 스킬명 형식도 호환한다.
...
$location-operator-writer 사무실-코덱스-딥시크!
$location-operator-writer 사무실-클로드-그록!
$location-operator-writer 사무실-클로드-딥시크!
$location-operator-writer 집-코덱스-그록!
$location-operator-writer 집-코덱스-딥시크!
$location-operator-writer 집-클로드-그록!
$location-operator-writer 집-클로드-딥시크!
```

명시적 스킬명을 붙이지 않는 호출도 같은 8개 조합을 사용한다.
...
```text
$location-operator-writer 사무실-코덱스-그록!
$location-operator-writer 집-클로드-딥시크! 이 버그를 수정해
```
...
- 장소 2개 × 오퍼레이터 2개 × 라이터 2개, 총 8개 조합을 모두 허용한다. 토큰이 유효하면 조합 자체를 제한하지 않는다.
- 라우트가 유효하더라도 선택한 장소의 실제 실행파일·wrapper·인증이 없으면 실행 단계에서 `BLOCKED` 또는 `CONFIG_DRIFT`로 보고한다. 이는 라우트 제한이 아니라 환경 검증 결과다.
- 메시지가 `^(집|사무실)-(코덱스|클로드|클로드코드)-(그록|딥시크)!(\s|$)` 규칙으로 시작하면 `$location-operator-writer` 없이도 이 스킬의 호출로 인식한다.
- 호출 마커 `!`는 파싱 후 제거하고 `ROUTE=사무실-코덱스-그록`처럼 보고한다.
- 라우트만 입력되면 `ROUTE_SELECTED`만 확인하고 모델·러너·작업방을 실행하지 않는다. 이후 같은 대화에서 작업 내용이 오면 마지막으로 선택한 라우트를 적용한다.
- 작업 내용과 함께 라우트가 입력되면 해당 메시지의 라우트가 이전 선택보다 우선한다.
- 호출명에 지정된 조합이 `PLACE`, `OPERATOR`, `WRITER`의 기준값이며, 실행 전 선택 보고에 그대로 표시한다.
- 활성 라우트가 없고 호출명에 라우팅이 없거나 토큰이 모호하면 장소·오퍼레이터·라이터를 추론하지 않는다. `ROUTE_REQUIRED` 또는 `ROUTE_INVALID`로 중단하고 올바른 호출 형식을 요청한다.
