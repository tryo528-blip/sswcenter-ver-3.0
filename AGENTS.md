...
- 본진에서는 Git 최상위가 `C:\sswcenter\3.0`인지 확인한다. 승인된 독립방에서는 Git 최상위가 그 task/thread에 배정된 worktree인지 확인한다.
...
- 허용된 Writer·검수 래퍼 호출에는 `C:\sswcenter\3.0` 또는 승인된 구현 worktree의 정확한 Git 최상위를 명시한다.
...
- `OPERATOR=CODEX`이면 각 독립방의 Codex가 자기 worktree에서 테스트·검수를 직접 수행한다. 본진과 독립방 모두 `invoke-codex.ps1`, `codex exec`, 자식 Codex 호출을 사용하지 않는다.
- `invoke-codex.ps1`는 `OPERATOR=CLAUDE_CODE`가 Codex에 테스트·검수를 위임할 때만 사용한다.
- `WRITER=GROK`이면 현재 오퍼레이터가 `invoke-grok.ps1`로 Grok에게 승인된 구현 범위만 맡긴다.
...
프롬프트 구성과 하네스 작성 지시는 [`00-오케스트레이션-작업지침.md`](00-오케스트레이션-작업지침.md)의 프롬프트 작성 원칙을 그대로 따르고 이 파일에 중복 정의하지 않는다.
