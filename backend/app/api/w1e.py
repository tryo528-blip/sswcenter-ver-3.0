...
ERROR_RESPONSES: dict[int | str, dict[str, Any]] = {
    ...
    409: {
        ...
        "description": (
            "CARE_ASSIGNMENT_PERIOD_CONFLICT, CARE_ASSIGNMENT_STAFF_INELIGIBLE, "
            "CARE_ASSIGNMENT_OUTSIDE_CONTRACT_PERIOD, CARE_ASSIGNMENT_OUTSIDE_EMPLOYMENT_PERIOD, "
            "or ROW_VERSION_CONFLICT"
        ),
    },
...
@router.get(
    "/recipients/{recipient_id}/contracts/{contract_id}/care-assignments",
    response_model=CareAssignmentListResponse,
    operation_id="listRecipientCareAssignments",
    responses=ERROR_RESPONSES,
)
def list_recipient_care_assignments(
    recipient_id: int,
    contract_id: int,
    ...
    del current_account
    return service.list_assignments(recipient_id, contract_id)
...
@router.post(
    ...
    operation_id="createRecipientCareAssignment",
    responses=ERROR_RESPONSES,
)
def create_recipient_care_assignment(
    recipient_id: int,
    contract_id: int,
...
@router.get(
    "/recipients/{recipient_id}/contracts/{contract_id}/care-assignments/{assignment_id}",
    response_model=CareAssignmentResponse,
    operation_id="getRecipientCareAssignment",
    responses=ERROR_RESPONSES,
)
def get_recipient_care_assignment(
    recipient_id: int,
    contract_id: int,
    assignment_id: int,
    current_account: RecipientViewAccountDependency,
...
@router.put(
    "/recipients/{recipient_id}/contracts/{contract_id}/care-assignments/{assignment_id}",
    response_model=CareAssignmentResponse,
    operation_id="replaceRecipientCareAssignment",
    responses=ERROR_RESPONSES,
)
def replace_recipient_care_assignment(
    recipient_id: int,
    contract_id: int,
    assignment_id: int,
