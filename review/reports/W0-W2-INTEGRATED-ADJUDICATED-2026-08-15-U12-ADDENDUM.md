...
> 구현 후보: `6ded847` (`fix(u12): revalidate retained correction staff`)
...
- 정정 중 원래 직원 fallback도 현재 draft의 재직·직위·GENERAL 자격 전체기간을
  다시 만족할 때만 유지한다.
...
- `6ded847` 이후 `/review` 결과와 지적사항 수정 후 최종 SHA를 다시 고정해야 한다. 이 부록은
